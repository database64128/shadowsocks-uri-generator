...
namespace ShadowsocksUriGenerator.CLI
{
    public static class UserCommand
    {
        ...
        public static async Task<int> ListCredentials(string[] usernames, string[] groups, CancellationToken cancellationToken = default)
        {
            ...
            List<(string username, string group, string method, string password)> filteredCreds = [];
        ...
        public static async Task<int> GetDataUsage(string username, SortBy? sortBy, CancellationToken cancellationToken = default)
        {
            ...
            switch (sortByInEffect)
            {
                ...
                case SortBy.NameAscending:
                    records = [.. records.OrderBy(x => x.group)];
                    break;
                case SortBy.NameDescending:
                    records = [.. records.OrderByDescending(x => x.group)];
                    break;
            ...
            if (users.UserDict.TryGetValue(username, out var user))
            {
                Console.WriteLine($"{"Data used",-16}{InteractionHelper.HumanReadableDataString1024(user.BytesUsed),-32}");
                if (user.BytesRemaining != 0UL)
                    Console.WriteLine($"{"Data remaining",-16}{InteractionHelper.HumanReadableDataString1024(user.BytesRemaining),-32}");
                if (user.DataLimitInBytes != 0UL)
                    Console.WriteLine($"{"Data limit",-16}{InteractionHelper.HumanReadableDataString1024(user.DataLimitInBytes),-32}");
            }
            ...
            if (records.All(x => x.bytesRemaining == 0UL)) // Omit data remaining column if no data.
            {
                ...
                {
                    Console.WriteLine($"|{group.PadRight(nameFieldWidth)}|{InteractionHelper.HumanReadableDataString1024(bytesUsed),11}|");
                }
            ...
            else
            {
                ...
                {
                    Console.Write($"|{group.PadRight(nameFieldWidth)}|{InteractionHelper.HumanReadableDataString1024(bytesUsed),11}|");
